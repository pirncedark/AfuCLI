...
type UpdateMethod = "brew" | "mise" | "nix" | "bun" | "npm" | "binary" | "source";
...
/**
 * afu-cli forku: bir script sarmalayicisinin (`afu.cmd` / `afu.sh`) icinden
 * calistirdigi `packages/coding-agent/src/cli.ts` yolunu okuyup depo kokunu
 * dondurur. Sarmalayici degilse, okunamiyorsa veya cozulen dizin bir git
 * deposu degilse `undefined` doner — o zaman normal (binary/npm) yol isler.
 */
function resolveSourceCheckoutDir(wrapperPath: string): string | undefined {
	...
	if (content.length > 64 * 1024) return undefined;
	const backslash = String.fromCharCode(92);
	const normalized = content.split(backslash).join("/");
	const marker = "/packages/coding-agent/src/cli.ts";
	const markerIndex = normalized.indexOf(marker);
	let repoDir: string | undefined;
	if (markerIndex >= 0) {
		// Yolun basini geriye dogru tara: tirnak, esittir veya bosluk sinirdir.
		const isBoundary = (ch: string) => ch <= " " || ch === '"' || ch === "'" || ch === "=";
		let begin = markerIndex;
		while (begin > 0 && !isBoundary(normalized[begin - 1] as string)) begin--;
		repoDir = path.resolve(normalized.slice(begin, markerIndex + marker.length), "../../../..");
	} else {
		// Eski sarmalayici bicimi: --cwd=<paket dizini>
		const cwdIndex = normalized.indexOf("--cwd=");
		if (cwdIndex >= 0) {
			let value = normalized.slice(cwdIndex + "--cwd=".length);
			if (value.startsWith('"')) {
			...
			if (value) repoDir = path.resolve(value, "../..");
		}
	}
	if (!repoDir) return undefined;
	return fs.existsSync(path.join(repoDir, ".git")) ? repoDir : undefined;
}
...
function resolveUpdateMethod(
	...
	// afu-cli forku: `afu` bir yerel checkout'u `bun .../src/cli.ts` ile calistiran
	// script sarmalayicisidir. Bu kontrol bun/npm ve Windows script-launcher
	// dallarindan ONCE gelmeli — sarmalayici onlara da benziyor.
	if (resolveSourceCheckoutDir(ompPath) !== undefined) return "source";
...
export function resolveUpdateTargetFromPath(
	...
	if (method === "source") return { method, path: ompPath };
	return { method };
}
/**
 ...
 */
/**
 * afu-cli forku: kaynak checkout'undan calisan kurulumu `git pull` + `bun install`
 * ile gunceller. Fork'ta yeni surumler `upstream` uzagindadir; duz `git pull`
 * yalnizca fork dalini tazeler ve yeni surume hic ulasamaz.
 */
async function updateViaSource(wrapperPath: string): Promise<void> {
	const repoDir = resolveSourceCheckoutDir(wrapperPath);
	if (!repoDir) {
		console.log(chalk.cyan("Please navigate to your source repository and run:"));
		console.log(chalk.cyan("  git pull && bun install"));
		return;
	}
	...
	try {
		...
		hasUpstream = remotes.includes("upstream");
		...
		const icon = theme?.status?.success ?? "OK";
		console.log(chalk.green(`\n${icon} Successfully updated source checkout at ${repoDir}`));
		console.log(chalk.dim(`Restart ${APP_NAME} to use the new version`));
	} catch (e) {
		console.log(chalk.red(`\nSource update failed: ${e instanceof Error ? e.message : String(e)}`));
		const pullCommand = hasUpstream ? "git pull --no-rebase upstream main" : "git pull";
		console.log(chalk.dim(`Please update manually: cd ${repoDir} && ${pullCommand} && bun install`));
	}
}
...
export async function runUpdateCommand(opts: {
	...
	try {
		...
		if (target.method === "nix") {
			...
			await updateViaSource(target.path);
